...
class groups_check extends \phpbb\cron\task\base
{
	...
	public function run()
	{
                ...
                $sql = "SELECT user_id, user_refreshToken, user_characterID FROM ".USERS_TABLE." WHERE user_type = ".USER_NORMAL." AND user_characterID IS NOT NULL AND (user_lastAPI < ".(time()-$this->config['groups_check_gc'])." OR user_lastAPI IS NULL) ORDER BY user_lastAPI ASC;";
                $result = $db->sql_query($sql);
                ...
                foreach ($dbusers as $dbu) {
                    //get current groups
                    if ($this->config['snitch_authevesso_req_validsso']) {
                    ...
                    $corpName = $corpinfo['corporation_name'];
                    ...
                    if (isset($corpinfo['alliance_id'])) {
                        ...
                        $allyName = EVEHELPERS::getAllyInfo($allyID)['alliance_name'];
                    ...
                    $sql = "SELECT forum_group FROM ".$table_prefix."authevesso_groups WHERE (type = 'corporation' AND id = ".$corpID.")";
                    if ($allyID != null) {
                    ...
                    if ($this->config['snitch_authevesso_req_corp'] && !count($newfgroups)) {
                            $sql = "UPDATE ".USERS_TABLE." SET user_type = ".USER_INACTIVE.", user_inactive_time = ".time().", user_inactive_reason = ".INACTIVE_REMIND." WHERE user_id = ".$dbu['user_id'];
                            $result = $db->sql_query($sql);
                ...
                if ($this->config['snitch_authevesso_enablets'] && count($tshandling)) {
                    try {
                        $ts3 = \TeamSpeak3::factory('serverquery://'.$this->config['snitch_authevesso_tsadmin'].':'.$this->config['snitch_authevesso_tspass'].'@'.$this->config['snitch_authevesso_tsserver'].':10011/?server_port='.$this->config['snitch_authevesso_tsport']);
                        $handlets = true;
                    } catch (\Exception $e) {
                        $handlets = false;
                    }
