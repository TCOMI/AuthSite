...
class main_module
{
    ...
    public function main($id, $mode)
    {
            ...
            foreach ($mappings as $map) {
                if (!empty($map['name']) && !empty($map['type']) && !empty($map['forum_group'])) {
                    if (empty($map['id'])) {
                        ...
                        $result = json_decode($searchapi->getSearch(array($map['type']), $map['name'], 'tranquility', 'en-us', true), true);
                        if (!isset($result[$map['type']]) || count($result[$map['type']]) == 0 ) {
                            ...
                            $results = $universeapi->postUniverseNames($result[$map['type']], 'tranquility');
                            foreach ($results as $r) {
                                if ($r->getName() == $map['name']) {
                                    $map['id'] = $r->getId();
                                }
                            }
                            if (!isset($map['id'])) {
        ...
        if (!empty($config['snitch_authevesso_tsadmin']) && !empty($config['snitch_authevesso_tspass'])) {
            try {
                $ts3 = \TeamSpeak3::factory('serverquery://'.$config['snitch_authevesso_tsadmin'].':'.$config['snitch_authevesso_tspass'].'@'.$config['snitch_authevesso_tsserver'].':10011/?server_port='.$config['snitch_authevesso_tsport']);
