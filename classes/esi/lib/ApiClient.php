...
class ApiClient
{
    ...
    public function callApi($resourcePath, $method, $queryParams, $postData, $headerParams, $responseType = null, $endpointPath = null)
    {
        ...
        // Make the request
        $tries = 0;
        $cachefile = realpath(dirname(__FILE__)).'/../../../../../../cache/esi_'.md5($url).'.json.gz';
        $isCached = False;
        if (file_exists($cachefile) && $method == self::$GET) {
            $cache = json_decode(gzdecode(file_get_contents($cachefile)), true);
            if(strtotime($cache['http_header']['Expires'])+self::$extracache >= strtotime("now")) {
                $isCached = True;
                $http_header = $cache['http_header'];
                $http_body = $cache['http_body'];
                $response_info = $cache['response_info'];
            }
        }
        if (!$isCached) {
            do {
                ...
                $tries++;
            } while ($tries < self::$maxtries && (curl_errno($curl) == 28 || $response_info['http_code'] >= 500 ));

            if ($method == self::$GET && isset($http_header['Expires'])) {
                $cache = array('http_header' => $http_header, 'http_body' => $http_body, 'response_info' => $response_info);
                file_put_contents($cachefile, gzencode(json_encode($cache), 9), LOCK_EX);
            }
        }
        ...
        if ($response_info['http_code'] == 0) {
            ...
            if (!empty($curl_error_message)) {
                $error_message = "API call to $url failed: $curl_error_message after $tries attempts.";
            } else {
